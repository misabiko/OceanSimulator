...
namespace Assets.Scripts.Boids
{
    ...
    // Custom component for per-instance float property
    ...
    public partial class BoidSystem : SystemBase
    {
        ...
        {
            // Define an EntityQuery to gather all entities with InstanceFloatProperty and LocalToWorld
            EntityQuery entityQuery = GetEntityQuery(typeof(AnimationTime), typeof(Position), typeof(Velocity), typeof(LocalToWorld));

            // Gather all matching entities into a NativeArray
            NativeArray<Entity> allEntities = entityQuery.ToEntityArray(Allocator.TempJob);

            // Gather all float properties for fast access
            NativeArray<Position> positions = entityQuery.ToComponentDataArray<Position>(Allocator.TempJob);
            NativeArray<Velocity> velocities = entityQuery.ToComponentDataArray<Velocity>(Allocator.TempJob);



            // Iterate over all entities with InstanceFloatProperty and LocalToWorld
            Entities.ForEach((Entity entity, int entityInQueryIndex, ref AnimationTime instanceFloat, ref Position pos, ref Velocity vel, ref Translation tr, ref Rotation rot, in LocalToWorld transform) =>
            {
                var id = entityInQueryIndex;
                ...
                var pos0 = pos.Value;
                var vel0 = vel.Value;
                var previousVelocity = vel.Value;

                //Example: Check against all other entities
                for (int i = 0; i < allEntities.Length; i++)
                {
                    if (allEntities[i] != entity) // Skip self
                    {
                        var pos1 = positions[i];
                        var vel1 = velocities[i];
                        var deltaPos = pos1.Value - pos0;
                ...
                /*
                Entities.ForEach((Entity entity1, int entityInQueryIndex, ref Position pos1, in Velocity vel1) =>
                {
                    if (entity != entity1)
                    {
                        var deltaPos = pos1.Value - pos0;
                ...
                }).Run(); //.WithDisposeOnCompletion(allEntities).WithDisposeOnCompletion(floatProperties).ScheduleParallel();
                */
                ...
                {
                    vel0 -= (avgPos - pos0) * Simulation.OvercrowdWeight; //  * (Simulation.MaxCountInProximity - countInProximity);
                }
                ...
                var asdf = float4x4.LookAt(pos0, pos0 + vel0, Vector3.up);
                //float4x4.Translate

                var dfas = Matrix4x4.LookAt(pos0, pos0 + vel0, Vector3.up);
                //transform.Value = asdf;
                //ref var tt = ref transform.Value;
                var t = asdf.Translation();
                var r = asdf.Rotation();

                

            }).WithDisposeOnCompletion(allEntities).WithDisposeOnCompletion(positions).WithDisposeOnCompletion(velocities).ScheduleParallel(); // Schedule the job for parallel execution
        }
